...
- [ ] **4. Provision the database.** Create a Neon or Supabase project (free tier) and obtain `DATABASE_URL`. Done when a test connection from local succeeds.
  - **BLOCKED (attempt 2)**: the earlier issue (URL pointed at the Supabase project/API host instead of the Postgres connection string) is fixed — `DATABASE_URL` now resolves to `db.<project-ref>.supabase.co:5432`, database `postgres`, which is a real Postgres endpoint (direct, not pooled). A throwaway `pg` connection test (script not committed, deleted after use; only non-secret metadata + pass/fail were ever printed) now reaches the server but gets rejected: Postgres error `28P01` — `password authentication failed for user "postgres"`. The password in `DATABASE_URL` is wrong, stale, or (a common cause) contains characters (`@ : / ? # %`) that need percent-encoding when embedded in a connection-string URL and weren't encoded, so the credential actually sent doesn't match the real password. Fix: in the Supabase dashboard, Settings → Database, either copy the connection string again via the provided "URI" copy button (which encodes it correctly) or reset the database password and rebuild `DATABASE_URL`, making sure any special characters in the password are percent-encoded. Tasks 5 and 6 were not attempted this round either — stopping in order per instruction.
