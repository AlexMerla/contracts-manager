...
- [ ] **4. Provision the database.** Create a Neon or Supabase project (free tier) and obtain `DATABASE_URL`. Done when a test connection from local succeeds.
  - **BLOCKED**: `DATABASE_URL` in `.env` is set to the Supabase *project* URL (`https://<project-ref>.supabase.co`, the host used for the REST/Auth JS client), not the Postgres *connection string* from the Supabase dashboard. A throwaway `pg` connection test (script not committed, deleted after use) confirmed this: the value parses with no port and no database path, and connecting times out (`ETIMEDOUT`) — that host doesn't listen on 5432/6543 at all. Fix: in the Supabase dashboard go to **Settings → Database → Connection string**, copy either the **direct** connection (`db.<project-ref>.supabase.co:5432`) or the **pooled/transaction** connection (`<pooler-host>:6543`, note: pooled does not support prepared statements, which `prisma migrate dev` in task 6 needs — prefer the direct 5432 string, or add both `DATABASE_URL` + `DIRECT_URL` per Prisma's documented pattern if pooling is required), and replace `DATABASE_URL` in `.env` with it. Tasks 5 and 6 were not attempted — task 6 needs a live connection, and the agent was instructed to implement 4/5/6 in order, stopping if one fails.
